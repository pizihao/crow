...
public class ParallelMulti {
  ...
  /**
   * 添加一个{@link Supplier} 并行执行，不影响其他任务的执行
   *
   * @param s Supplier
  ...
  /**
   * 添加一个{@link IntSupplier} 并行执行，不影响其他任务的执行
   *
   * @param s IntSupplier
   * @return com.deep.crow.task.parallel.ParallelMulti
   * @author liuwenhao
  ...
  /**
   * 添加一个{@link LongSupplier} 并行执行，不影响其他任务的执行
   *
  ...
  /**
   * 添加一个{@link DoubleSupplier} 并行执行，不影响其他任务的执行
   *
  ...
  /**
   * 添加一个{@link Runnable} 并行执行，不影响其他任务的执行<br>
  ...
  /**
   * 添加一个异常任务节点 为处于[start,end]之间的任务添加异常处理
   *
   * @param start 开始序号
   * @param end 结束序号
   * @param fn 异常任务
  ...
  /**
   * 添加一个异常任务节点 如果还有任务没有被设置异常处理节点，则添加<br>
  ...
  /**
   * 额外添加一个任务 在multiList的队尾添加一个并行执行的任务过程
   *
   * @param multi 并行的任务执行
   * @return com.deep.crow.task.parallel.ParallelMulti
  ...
  /**
   * 额外添加一个任务的执行 在multiList的队尾添加一个并行执行的任务过程
   *
  ...
  public <T> Multi<T> multiForIndex(int index) {
    ...
    MultiOrder<?> multiOrder =
        multiList.stream().filter(m -> m.getOrder() == index).findFirst().orElse(null);
  ...
  /**
   * 获取结果 如果存在多个符合条件的则组成集List集合返回<br>
   * 不适用于存在泛型的情况
   *
   * @return T 一个未知的类型
   * @author Created by liuwenhao on 2022/4/12 23:16
  ...
  /**
   * 获取结果 如果存在多个符合条件的则组成List集合返回<br>
   *
   * @return T 一个未知的类型
   * @author Created by liuwenhao on 2022/4/12 23:16
   */
